    ...
    <div class="py-8">
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white shadow-sm sm:rounded-xl p-6 md:p-8">
                ...
                @endif
                <form action="{{ route('tenant.admin.incomes.store') }}" method="POST" class="space-y-6">
                    @csrf
                    <div>
                        <label for="title" class="block text-sm font-semibold text-gray-700 mb-1">Título <span class="text-red-500">*</span></label>
                        <input type="text" id="title" name="title" value="{{ old('title') }}" placeholder="Ej: Venta del día, Cobro cliente..." autofocus class="w-full border-gray-300 rounded-lg shadow-sm text-sm px-4 py-2.5">
                    </div>
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label for="amount" class="block text-sm font-semibold text-gray-700 mb-1">Monto (S/) <span class="text-red-500">*</span></label>
                            <input type="number" id="amount" name="amount" value="{{ old('amount') }}" step="0.01" min="0.01" placeholder="0.00" class="w-full border-gray-300 rounded-lg shadow-sm text-sm px-4 py-2.5">
                        </div>
                        <div>
                            <label for="date" class="block text-sm font-semibold text-gray-700 mb-1">Fecha <span class="text-red-500">*</span></label>
                            <input type="date" id="date" name="date" value="{{ old('date', date('Y-m-d')) }}" class="w-full border-gray-300 rounded-lg shadow-sm text-sm px-4 py-2.5">
                        </div>
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <textarea id="description" name="description" rows="3" placeholder="Detalle adicional..." class="w-full border-gray-300 rounded-lg shadow-sm text-sm px-4 py-2.5 resize-none">{{ old('description') }}</textarea>
                    </div>
                    <div class="flex gap-3 pt-2">
                        <button type="submit" class="flex-1 bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-3 rounded-lg text-sm transition">💾 Guardar Ingreso</button>
