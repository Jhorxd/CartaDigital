<x-app-layout>
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
                <h2 class="font-black text-2xl text-slate-800 tracking-tight flex items-center gap-2">
                    <span class="p-2 bg-indigo-100 text-indigo-600 rounded-xl">⚡</span> Entrada Rápida de Gastos
                </h2>
                <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-1">Modo Hoja de Cálculo (Alta Eficiencia)</p>
            </div>
            <a href="{{ route('tenant.admin.expenses.index') }}" class="text-slate-400 hover:text-slate-800 text-xs font-black uppercase tracking-widest transition-colors flex items-center gap-2">
    ...
    <div class="py-8 bg-slate-50/50 min-h-screen" x-data="quickExpenses('{{ $lastDate }}', {{ json_encode($categories) }})">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">

            @if (session('error'))
                <div class="mb-6 bg-rose-50 border border-rose-100 text-rose-600 p-4 rounded-2xl font-bold text-sm shadow-sm">{{ session('error') }}</div>
            @endif
            ...
            <div class="bg-white rounded-[2rem] shadow-sm border border-slate-100 mb-6 p-4 flex flex-wrap gap-4 items-center justify-between">
                <div class="flex flex-wrap gap-2 items-center">
                    <button type="button" @click="addRow()" class="flex items-center gap-2 text-xs font-black bg-indigo-600 text-white px-6 py-3 rounded-xl hover:bg-indigo-700 transition shadow-lg shadow-indigo-100 active:scale-95">
                ...
                <div class="flex items-center gap-6 w-full sm:w-auto justify-between sm:justify-end border-t sm:border-t-0 pt-4 sm:pt-0">
                    <div class="text-right">
                        <div class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Total Acumulado</div>
                    ...
                    <button type="button" @click="submitAll()"
                        class="bg-emerald-600 hover:bg-emerald-700 text-white font-black px-10 py-4 rounded-2xl text-sm transition shadow-xl shadow-emerald-100 transform active:scale-95">
                        💾 Guardar Todo
                    </button>
                </div>
            </div>
            ...
            <div class="bg-white rounded-[2.5rem] shadow-sm border border-slate-100 overflow-hidden">
                <form id="bulk-form" action="{{ route('tenant.admin.expenses.bulk') }}" method="POST">
                    @csrf
                ...
                <div class="hidden md:grid gap-0 border-b border-slate-100 bg-slate-50/50 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]"
                    ...
                    <div class="px-2 py-4 flex items-center justify-center">
                        <input type="checkbox" @change="toggleAll($event)" class="rounded-lg border-slate-200 text-indigo-600 focus:ring-indigo-500 w-5 h-5">
                    </div>
                    <div class="px-4 py-4">Título <span class="text-rose-400">*</span></div>
                ...
                <div class="divide-y divide-slate-50">
                    <template x-for="(row, index) in rows" :key="row.id">
                        <div
                            ...
                            :class="{ 'bg-indigo-50/30': row.focused, 'bg-rose-50/50': row.selected, 'bg-white': !row.focused && !row.selected }"
                        >
                            {{-- Vista Desktop --}}
                            <div class="hidden md:grid gap-0 items-center group" style="grid-template-columns: 60px 1fr 140px 160px 180px 1fr 60px;">
                                ...
                                <div class="border-r border-slate-50">
                                    <input type="text" x-model="row.title" placeholder="¿En qué gastaste?"
                                ...
                                <div class="border-r border-slate-50">
                                    <input type="number" x-model="row.amount" placeholder="0.00" step="0.01" min="0"
                                        ...
                                        class="w-full px-4 py-5 text-sm border-0 focus:ring-0 bg-transparent outline-none text-right font-black text-rose-600 placeholder:text-rose-200">
                                </div>
                            ...
                            <div class="md:hidden p-6 space-y-4">
                                <div class="flex justify-between items-center">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <span class="text-[10px] font-black text-slate-300 uppercase tracking-[0.2em]" x-text="'Gasto #'+(index+1)"></span>
                                    </div>
                                ...
                                <div class="space-y-4 bg-slate-50/50 p-4 rounded-3xl border border-slate-100">
                                    <div class="grid grid-cols-12 gap-4">
                                        <div class="col-span-12">
                                            <label class="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1 block">¿En qué gastaste?</label>
                                            <input type="text" x-model="row.title" placeholder="Ej: Insumos cocina" class="w-full bg-white border-slate-200 rounded-xl text-sm px-4 py-3 font-black text-slate-800 focus:ring-rose-500 focus:border-rose-500">
                                        </div>
                                    </div>
                                    ...
                                    <div class="grid grid-cols-2 gap-4">
                                        <div>
                                            ...
                                            <input type="number" x-model="row.amount" placeholder="0.00" class="w-full bg-white border-slate-200 rounded-xl text-sm px-4 py-3 text-right font-black text-rose-600 focus:ring-rose-500 focus:border-rose-500">
                                        </div>
                                        <div>
                                            <label class="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1 block">Fecha</label>
                                            <input type="date" x-model="row.date" class="w-full bg-white border-slate-200 rounded-xl text-xs px-4 py-3 font-bold text-slate-600 focus:ring-rose-500 focus:border-rose-500">
                                        </div>
                                    </div>

                                    <div class="grid grid-cols-1 gap-4">
                                        <div>
                                            <label class="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1 block">Categoría</label>
                                            <select x-model="row.category" class="w-full bg-white border-slate-200 rounded-xl text-xs px-4 py-3 font-black text-slate-600 uppercase tracking-widest focus:ring-rose-500 focus:border-rose-500">
                                                <option value="">Sin Categoría</option>
                                                <template x-for="cat in categories" :key="cat">
                                                    <option :value="cat" x-text="cat"></option>
                                                </template>
                ...
                <div class="p-6 bg-slate-50/30 border-t border-slate-100">
                    <button type="button" @click="addRow()" class="w-full md:w-auto flex items-center justify-center gap-3 px-10 py-5 bg-white border-2 border-dashed border-slate-200 text-slate-400 rounded-3xl hover:border-indigo-400 hover:text-indigo-600 transition group">
                        <span class="text-xl group-hover:scale-125 transition-transform">➕</span>
                        <span class="font-black text-xs uppercase tracking-widest">Agregar nueva fila de gasto</span>
                    </button>
                </div>
            </div>
            ...
            <div class="mt-10 flex flex-col sm:flex-row justify-between items-center gap-6">
                <div class="flex items-center gap-4">
                    <div class="flex -space-x-2">
                         <div class="w-10 h-10 rounded-full bg-rose-100 flex items-center justify-center border-2 border-white text-rose-600 font-black text-xs" x-text="rows.length"></div>
                         <div class="w-10 h-10 rounded-full bg-emerald-100 flex items-center justify-center border-2 border-white text-emerald-600 font-black text-xs" x-text="rows.filter(r => r.title && r.amount).length"></div>
                    </div>
                ...
                <div class="flex gap-4 w-full sm:w-auto">
                    <a href="{{ route('tenant.admin.expenses.index') }}" class="flex-1 sm:flex-none text-center px-10 py-5 bg-white border border-slate-200 text-slate-400 rounded-3xl text-xs font-black uppercase tracking-widest hover:bg-slate-50 transition active:scale-95">
                        Cancelar
                    </a>
                    <button type="button" @click="submitAll()"
                        class="flex-1 sm:flex-none px-16 py-5 bg-slate-900 hover:bg-black text-white font-black rounded-[2rem] text-xs uppercase tracking-widest transition shadow-2xl transform active:scale-95">
    ...
    <script>
    function quickExpenses(lastDate, categories) {
